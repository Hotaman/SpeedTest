...
self.addEventListener("fetch", event => {
    ...
    let modifiedUrl =
        typeof urlRewritingHook === "function" ? urlRewritingHook(event.request.url) : null;
    let options = {};
    ...
    event.respondWith(
        fetch(modifiedUrl || event.request, options).then(function(response) {
...
class HowSlowForSW {
    ...
    getBandwidth() {
        ...
        if (
            !this.connectionTypeFromDatabase ||
    ...
    splitTimingIntoIntervals(timing) {
        let startingBlock = Math.floor(
            (timing.responseStart - this.epoch) / this.INTERVAL_DURATION,
        );
    ...
    estimateBandwidthForAPeriod(numberOfSeconds) {
        ...
        const nineteenthPercentile = this.percentile(newArray, 0.9);
    ...
    estimateRTTForAPeriod(numberOfSeconds) {
        ...
        return this.percentile(pings, 0.2);
    }
    ...
    retrieveStats() {
        try {
            this.database
                .transaction("bw", "readonly")
                .objectStore("bw")
                .get(1).onsuccess = event => {
